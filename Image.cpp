...
void Image::Render(HDC hdc, int destX, int destY, int sourX, int sourY, int sourWidth, int sourHeight, float scale)
{
	if (isTrans)
	{
		GdiTransparentBlt(
...
void Image::AnimationRender(HDC hdc, int destX, int destY, Animation * ani, float scale)
{
	Render(hdc, destX, destY, ani->GetFramePos().x, ani->GetFramePos().y, imageInfo->keyFrameWidth, imageInfo->keyFrameHeight, scale);
}
