...
public:
	...
	void Render(HDC hdc, int destX, int destY, int sourX, int sourY, int sourWidth, int sourHeight, float scale = 1.0f);
	...
	// 애니메이션									포인터로 애니메이션 가져오기!.
	void AnimationRender(HDC hdc, int destX, int destY, Animation* ani, float scale = 1.0f);
	...
	// 전체 크기랑 프레임이랑 하는 벡터를 만들어줬고 이걸 Camel Init에서 사용해보자!
	...
	void SetIsMapImage(bool _isMapImage) { isMapImage = _isMapImage; }
	Image();
	~Image();
};
